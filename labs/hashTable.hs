...
insertToTable (ToHashTable table c l e) key value  = checkLF $ updateLF $ ToHashTable
                                 ([ if mod (hash key) c  == fst li
                                         then (snd li) ++ [(key,value)]
                                    else snd li | li <- enumerate table
                                   ])
                                 c l (e + 1) 
...
erase (ToHashTable table c l e) key = ToHashTable (map
                               ( \(i,o) -> if i == mod (hash key) c then [el| el<-o, fst el /= key] else o ) (enumerate table) )
                                 c l (length $ concat table)
...
at table key | contains table key = Just $ head [v | (k,v) <- (getSamelyHashed table key) ] 
             | otherwise = Nothing
